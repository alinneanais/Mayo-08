...
<?php for($n = 0; $n < $todos; $n++){?>
    ...
    <?php if (substr_count($datos[$n]['data'][''],'2017-05-08') > 0){?>
      <li>El Smart Citizen Kit <?php echo($datos[$n]['id']);?> está <?php echo('instalado en '.$datos[$n]['city']);?> (<a href="https://www.google.cl/maps/place/<?php echo($datos[$n]['latitude'].','.$datos[$n]['longitude']);?>" rel="nofollow" target="_blank">Ver ubicación en Google Maps</a>,
        <a href="php_json_detalles.php?url=/<?php echo rawurlencode('https://api.smartcitizen.me/v0/devices'.$datos[$n]['id'].'.json'); ?>">Ver detalles</a>).</li>
    <?php };?>
  <?php };?>
<?php };?>
</ol>
